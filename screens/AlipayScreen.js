...
import { View, Text, ActivityIndicator, Alert, StyleSheet, TouchableOpacity, Linking } from 'react-native';
        ...
        onShouldStartLoadWithRequest={(request) => {
          ...
          if (request.url.startsWith('alipays://')) {
            Linking.openURL(request.url).catch(err =>
              Alert.alert('Alipay', i18n.t('cannotOpenAlipay') || 'Unable to open Alipay app')
            );
            return false; // stop WebView from handling it
          }
          return true;
        }}
      />
