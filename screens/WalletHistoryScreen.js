...
export default function WalletHistoryScreen() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        if (!userId) {
          Alert.alert('❌', i18n.t('walletErrors.noUserId'));
          return;
        }
        ...
        if (Array.isArray(res.data?.history)) {
          ...
          Alert.alert('⚠️', i18n.t('walletErrors.noRechargeHistory'));
        }
      } catch (err) {
        console.error('❌ History fetch error:', err.message);
        Alert.alert('❌', i18n.t('walletErrors.failedToLoadHistory'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const renderItem = ({ item }) => {
    ...
    const formattedDate = date ? new Date(date).toLocaleString() : i18n.t('walletHistory.unknownDate');
    const amount = item.amount ?? 0;
    const method = item.method ?? i18n.t('walletHistory.recharge');
  ...
  return (
    <SafeAreaView style={styles(isDark).container}>
      <Text style={styles(isDark).title}>📜 {i18n.t('walletHistory.title')}</Text>

      {loading ? (
        <ActivityIndicator size="large" color="#f8e1c1" style={{ marginTop: 20 }} />
      ) : history.length === 0 ? (
        <Text style={styles(isDark).emptyText}>{i18n.t('walletHistory.empty')}</Text>
      ) : (
        <FlatList
          data={history}
