...
export default function RechargeScreen() {
  ...
  const handlePaypalPay = async (amount) => {
    ...
    try {
      ...
      if (res.data?.approvalUrl) {
        ...
        Alert.alert('PayPal', res.data.error || i18n.t('missingPaymentUrl'));
      }
    } catch (err) {
  ...
  const handleApplePay = async (amount) => {
    if (!amount || isNaN(amount) || amount < 10) {
      return Alert.alert(i18n.t('invalidAmount'), i18n.t('minimumAmount') + ' 10 USD.');
    }
    ...
    try {
      ...
      if (res.data?.paymentUrl) {
        ...
        Alert.alert('Apple Pay', res.data.error || i18n.t('missingPaymentUrl'));
      }
    } catch (err) {
  ...
  return (
    <ScrollView contentContainerStyle={styles(isDark).container}>
      ...
      <Text style={styles(isDark).label}>{i18n.t('currentBalance')}</Text>
      <Text style={styles(isDark).balance}>{balance} {i18n.t('currency')}</Text>

      <Text style={styles(isDark).label}>{i18n.t('enterAmount')}</Text>
      <TextInput
        placeholder={i18n.t('enterAmountPlaceholder')}
...
>
  <Text style={[styles(isDark).buttonText, { color: '#fff' }]}>
    WeChat Pay {manualAmount || '...'} {i18n.t('currency')}
      ...
      >
        <Text style={[styles(isDark).buttonText, { color: '#fff' }]}>
          PayPal {manualAmount || '...'} {i18n.t('currency')}
        </Text>
      </TouchableOpacity>
      ...
      >
        <Text style={[styles(isDark).buttonText, { color: '#fff' }]}>
           Apple Pay {manualAmount || '...'} {i18n.t('currency')}
        </Text>
      </TouchableOpacity>
      ...
      >
        <Text style={[styles(isDark).buttonText, { color: '#f8e1c1', fontSize: 14 }]}>
          📜 {i18n.t('viewRechargeHistory')}
        </Text>
      </TouchableOpacity>

      {loading && <ActivityIndicator size="large" color="#f8e1c1" style={{ marginTop: 20 }} />}

      <Text style={styles(isDark).note}>💡 {i18n.t('rechargeNote')}</Text>
    </ScrollView>
  );
}
